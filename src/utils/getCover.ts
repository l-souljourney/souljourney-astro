import fs from 'node:fs/promises';
import path from 'node:path';
import SITE_INFO from '@/config';

// 支持的图片扩展名
const SUPPORTED_EXTENSIONS = new Set(['.jpg', '.jpeg', '.png', '.gif', '.webp', '.bmp']);
const BANNER_RELATIVE_DIR = path.join('public', 'assets', 'images', 'banner');
const OBSIDIAN_WIKILINK_RE = /^\[\[[^[\]]+\]\]$/;

export function resolveBannerDir(cwd: string = process.cwd()) {
  return path.resolve(cwd, BANNER_RELATIVE_DIR);
}

/**
 * 获取目录中的所有图片文件
 * @param {string} dir - 图片目录路径
 * @returns {Promise<string[]>} - 图片文件名数组
 */
async function getImageFiles(dir: string): Promise<string[]> {
  try {
    const files = await fs.readdir(dir);
    return files.filter(file => SUPPORTED_EXTENSIONS.has(path.extname(file).toLowerCase()));
  } catch (error) {
    console.error(`无法读取目录 ${dir}:`, error);
    return [];
  }
}

/**
 * 随机打乱数组（Fisher-Yates 洗牌算法）
 * @param {T[]} array - 需要打乱的数组
 * @returns {T[]} - 打乱后的数组
 */
function shuffleArray<T>(array: T[]): T[] {
  for (let i = array.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [array[i], array[j]] = [array[j], array[i]];
  }
  return array;
}

/**
 * 创建一个图片名称的无限迭代器
 * @param {string} dir - 图片目录路径
 * @returns {AsyncGenerator<string>} - 异步生成器
 */
async function* createImageIterator(dir: string): AsyncGenerator<string, never, void> {
  let images = await getImageFiles(dir);
  if (images.length === 0) {
    throw new Error(`目录 ${dir} 中没有图片文件`);
  }

  while (true) {
    images = shuffleArray([...images]); // 打乱顺序
    for (const image of images) {
      yield image;
    }
  }
}

const fileIter = createImageIterator(resolveBannerDir());

export default async function getCover(filename: string | null | undefined): Promise<string> {
  const normalizedFilename = filename?.trim();
  if (normalizedFilename && !OBSIDIAN_WIKILINK_RE.test(normalizedFilename)) {
    return normalizedFilename;
  }
  const { value } = await fileIter.next();
  return SITE_INFO.Site + `/assets/images/banner/${value}`;
}
