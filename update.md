...
- 两个陈旧任务按用户决定结束为 obsolete，由后续统一项目规划与 Issue 取代，不表示原业务验收已完成。
- 退出项目级 Trellis 技能与客户端注入，保留冷历史；不改变站点产品代码、内容集合或生产发布行为。

## v2.3.4 (2026-05-03) - 公开文档治理与历史材料收敛

### 🧭 公开入口与版本口径统一
- **版本补丁立项**: 新增 `v2.3.4`，将本轮工作明确为“公开文档治理补丁版”，不混入 `v2.4` 的 Astro 升级范围。
- **主入口口径统一**: 对齐 `README.md`、`docs/README.md`、路线图与 Trellis 任务记录，消除 `v2.3.0 / v2.3.1 / v2.3.3` 混写造成的当前状态漂移。

### 🧹 文档公开深度收缩
- **活跃文档降到中层说明**: 活跃公开文档继续保留架构、职责边界、发布契约与协作语义，但不再默认承载 secrets 名称、平台地址、运行证据和运维执行细节。
- **历史材料分级治理**: 明确区分“活跃公开文档 / 历史归档 / 仅保留摘要 / 不继续公开”的治理模型，开始收敛不适合长期公开暴露的资料。
- **Trellis archive 纳入治理**: 将 `.trellis/tasks/archive/**` 纳入公开面审查范围，避免历史研究记录继续暴露内部运维上下文。

### ✅ 当前阶段目标
- 统一当前公开入口与版本快照
- 收敛 deploy / integration 文档的公开深度
- 为历史 docs 与 Trellis archive 建立可执行的摘要化与移除规则
...
### 🛡️ SEO & 路由守卫
- **英文分类页 robots 策略修正**: 仅在空分类场景显式输出 `noindex, follow`，非空分类回退 Head 默认策略，不再页面级硬编码 `index, follow`。
- **移动端英文前缀幂等保护**: `withLangPrefix` 增加 `/en` 已前缀路径直返逻辑，避免 `/en/en/*` 重复前缀。

### ✅ 验证
- `node --test tests/v2.1.3-regression.test.mjs` 通过
- `pnpm build` 通过

---

## v2.1.3 (2026-03-25) - 双语交互一致性与语义/索引补强

### 🔧 交互一致性修复
- **移动端侧边栏**: 修复英文文案 key 泄露与英文路径前缀不一致问题。
- **TOC 高亮**: 修复目录高亮选择器与文章结构不匹配导致的高亮漂移。
- **搜索弹层监听器**: 修复多次切页后重复绑定导致的重复触发。

### 🧱 结构与索引修复
- **单主地标约束**: zh/en 文章页移除内层 `main`，保证仅保留一个页面级主地标。
- **英文空分类页策略**: 补充 empty-state 文案，并为空分类输出 `noindex, follow`。
- **英文 canonical 分类可达性**: 空数据分类页不再返回 404。

---

## v2.1.2 (2026-03-25) - 文章结构与 SEO 核心热修复

### 🧩 结构闭合修复
- 修复 zh/en 文章页模板标签闭合与语义容器错配问题。

### 🧠 SEO 元数据链路修复
- 统一 `Layout -> Head` 的封面与页面类型传递。
- 文章页显式输出 `pageType="article"`，修复 `og:type` 与 JSON-LD 类型一致性。

### 🌍 英文分类入口修复
- 英文分类静态路径合并 canonical 分类全集，空数据时输出有效页面（非 404）。

---

## v2.1.1 (2026-03-24) - SEO 与内容完整性守卫

### 🧪 构建期守卫
- 新增正文起始重复 frontmatter 检测，命中即构建失败。
- `description` 解析优先 frontmatter 字段，回退正文前先执行污染检测。

### 🌐 语言切换与 hreflang 守卫
- 仅在存在对侧稿件时输出 `hreflang` 与文章页语言切换目标，避免指向 404。
...
- **页脚优化**: 修复了 ICP 备案号和 Astro 徽章文字显示为空的问题。
  - 优化了徽章渲染逻辑，当翻译缺失时自动回退显示原始标签。
  - 将 ICP 备案号整合进徽章显示，移除了底部冗余的文字信息。

## v1.9.12 (2025-12-25) - 细节体验与版权优化

### 🐛 Bug Fixes
- **文章目录 (TOC)**: 修复英文模式下侧边栏目录标题仍显示中文的问题，实现 "Table of Contents" 国际化。
- **页脚 (Footer)**: 修复 RSS 和 Sitemap 徽章文字标签不可见的问题，优化样式防止被截断。

### ⚡ Optimizations
- **版权声明 (Copyright)**: 优化文章底部 CC BY-NC-SA 4.0 版权声明文案，采用更严谨的法律措辞，并实现完整的中英文国际化。

---

## v1.9.11 (2025-12-23) - 紧急修复构建失败

### 🐛 Bug Fixes
- **版权组件**: 修复 `Copyright.astro` 中错误的 Frontmatter 语法（多重代码块导致变量未定义），解决 CI 构建失败问题。

---

## v1.9.10 (2025-12-23) - 导航栏国际化补全

### 🐛 Bug Fixes
- **导航栏**: 修复导航栏菜单项未正确应用国际化翻译的问题（从硬编码中文迁移到动态 Key）。

---

## v1.9.9 (2025-12-23) - 国际化深度修复与合规性增强

### 🐛 Bug Fixes
- **文章归档**: 彻底修复归档页面语言过滤问题，引入 `lang` Frontmatter 字段，不再单纯依赖 ID 前缀，确保中英文文章严格隔离。
- **页脚合规**: 修复由国际化重构导致的 ICP 备案号丢失问题，新增显式备案信息栏，确保合规展示。
- **版权组件**: 修复文章底部版权声明区域的中文硬编码问题，实现完整国际化。

### ⚡ Improvement
- **构建配置**: 修订 content collection schema，支持显式语言定义，为多语言共用 ID 架构提供底层支持。

---

## v1.9.8 (2025-12-23) - 细节优化与稳定性提升

### 🐛 Bug Fixes
- **Archive Links**: 修正归档页文章链接生成逻辑，优先读取 Frontmatter ID，解决部分文章 404 问题。
- **Category Count**: 修复中文分类页面错误统计所有语言文章数量的问题，实现严格的语言隔离。
- **English 404**: 新增 404 页面客户端脚本，访问英文路径时自动切换为英文提示与首页链接。
- **Data Consistency**: 修复部分文章文件名与 ID 不一致导致的死链问题。

### ⚡ Infrastructure
- **Playwright Upgrade**: 升级自动化测试架构，配置独立测试端口 (4322) 与复用策略，大幅提升测试稳定性。
- **Link Crawler**: 新增全站深度链接爬虫测试，精准捕获死链。
...
### 🌍 i18n Optimizations
- **侧边栏 (Sidebar)**: 彻底重构侧边栏组件 `Aside.astro`，实现数据源的语言隔离。英文版侧边栏现在仅显示英文文章的统计数据、分类列表、标签云和推荐文章，不再与中文内容混杂。
- **归档页 (Archives)**: 优化中文归档页面逻辑，严格过滤非中文文章（如 `en/` 开头的内容），确保归档列表的纯净性。
- **页脚 (Footer)**: 扩展页脚配置能力，支持徽章 Label 的国际化翻译（如 "Sitemap" 在中文版自动显示为 "网站地图"）。
- **数据层 (Data Layer)**: 增强 `getPostInfo` 工具库，为核心数据获取函数增加 `lang` 参数支持，奠定多语言数据隔离的基础。
...
### 🐛 Fixed & Refactored
- **国际化架构**: 深度重构 `Layout`, `Head`, `Header`, `Footer` 等核心组件，全面替换硬编码字符串为 `t()` 动态翻译，实现更彻底的本地化。
- **英文归档页**: 修复了英文归档页面 (`/en/archives`) 因数据结构不匹配导致的 500 错误，并修正了排序逻辑。
- **RSS**: 修复了英文 RSS Feed (`/en/rss.xml`) 的描述字段显示问题。
- **配置管理**: 重构 `config.ts` 中的导航配置，改为使用翻译键管理，提升可维护性。

---

## v1.9.3 (2025-12-05) - 国际化功能完善

### ✨ Features
- **归档页面**: 新增英文归档页 `/en/archives`，支持按年份分组显示英文文章
- **RSS 订阅**: 实现 RSS 订阅源分离，`/en/rss.xml` 仅包含英文内容，`/rss.xml` 保持中文内容
- **Sitemap**: 优化站点地图生成，完整支持多语言路由索引

### ⚡ Improvements
- **SEO**: 为不同语言页面配置了对应的 RSS 自动发现链接
- **组件优化**: 增强 Archive 组件的健壮性，修复无标签文章可能导致的构建错误

---

## v1.9.2 (2025-12-05) - 国际化质量修复

### 🐛 Bug Fixes
- **分类系统**: 修复英文页面分类名称未翻译及链接错误的问题
- **语言隔离**: 修复中文分类页错误显示英文文章的问题
- **归档组件**: 修复归档列表文章链接缺少语言前缀的问题
- **日期格式**: 统一英文首页文章卡片的日期显示格式
- **UI 交互**: 修复移动端语言切换按钮可能被搜索层遮挡的问题

### ⚡ Improvements
- **构建稳定性**: 修复分类页面构建时的语法和引用错误
- **代码质量**: 优化国际化工具函数的引用方式
...
在 v1.9.0 基础上补齐核心页面多语言支持，优化导航配置，增强日期格式化。

#### 功能清单

- **导航配置优化**: 使用映射表替代硬编码，新增 `nav.archives` 和 `nav.friends` 翻译键
- **404 页面国际化**: 创建英文 404 页面 (`/en/404`)
- **关于页面国际化**: 创建英文关于页面 (`/en/about`)
- **分类页国际化**: 5 个英文分类页面，支持按年份分组
- **标签页国际化**: 自动生成所有英文标签页面
- **日期格式增强**: 新增 `formatDate` 工具，中英文日期完全本地化
  - 中文：`2025年12月03日` 或 `2025-12-03`
  - 英文：`December 3, 2025` 或 `Dec 3, 2025`
...
实现中英双语全栈支持，从配置、路由、UI到SEO的完整国际化方案。

#### 功能清单

- **i18n 模块启用**: Astro 原生 i18n 支持，默认中文（`/`），英文（`/en`）
- **翻译基础设施**: 创建 `src/i18n/ui.ts` 翻译字典（30+ 键）和 `src/i18n/utils.ts` 工具函数
- **路由重构**: 
  - 中文路由：`/` 和 `/article/{id}`
  - 英文路由：`/en/` 和 `/en/article/{id}`
- **UI 组件国际化**: Header（语言切换）、Footer、ArticleCard、Pagination 全部支持多语言
- **SEO 优化**: 添加 hreflang 标签、动态 og:locale、语言切换 URL 生成
- **内容结构调整**: 支持 `published/zh/` 和 `published/en/` 目录结构

#### 构建结果

- 页面数量：**23 页**（包括 `/en/index.html`）
- 新增文件：4 个
- 修改文件：8 个
- 代码行数：~500 行
