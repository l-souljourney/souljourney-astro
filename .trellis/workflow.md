...
- Codex 在本仓库默认使用 `inline` / 主会话执行 Trellis；只有用户明确要求并行、子代理、分工或 delegate 时，才允许派生 sub-agent。
- 正式开发任务继续执行 Trellis task / planning artifacts / Change / Verify / spec 沉淀闭环；GitHub Issue、PR、Milestone 和 Project 按需使用，不作为任务完成前置条件。
