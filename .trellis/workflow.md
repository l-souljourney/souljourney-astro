...
|---|---|
| Wants a new feature / requirement unclear | `trellis-brainstorm` |
| About to write code / start implementing | `trellis-before-dev`, then implement in the main session by default |
| Finished writing / want to verify | `trellis-check` in the main session by default |
| Stuck / fixed same bug several times | `trellis-break-loop` |
| Spec needs update | `trellis-update-spec` |

**Why Codex is different in this repo:** this repository uses Codex main-session execution as the default Trellis mode. Only dispatch sub-agents when the user explicitly asks for parallel/delegated work and the environment supports the required Trellis agent type.
...
Do research in the main session by default and write findings into `{TASK_DIR}/research/`.
If the user explicitly requests parallel/delegated research and the environment supports the Trellis research agent, dispatch `trellis-research`.
...
If this task will use delegated Trellis sub-agents later, curate `implement.jsonl` and `check.jsonl` now.
If this task will execute in the main session only, `prd.md` plus `research/` artifacts are sufficient; add JSONL entries only for material a delegated sub-agent would need.
...
| `implement.jsonl` has agent-curated entries | required only when delegated sub-agents will be used |
...
1. Load the `trellis-before-dev` skill and read project guidelines.
2. Read `{TASK_DIR}/prd.md` and `info.md` if present.
3. Consult materials under `{TASK_DIR}/research/`.
4. Implement in the main session.
5. Run project lint and type-check.

If the user explicitly asks for parallel/delegated implementation and the environment supports `trellis-implement`, dispatch it with curated `implement.jsonl`.
...
Load the `trellis-check` skill and verify in the main session:
- Spec compliance
- lint / type-check / tests
- Cross-layer consistency (when changes span layers)

If the user explicitly asks for delegated review/fixup and the environment supports `trellis-check`, dispatch it after curating `check.jsonl`.
...
1. **Inspect dirty state**:
   ...
   In this repository, confirm the task is on the intended working branch and that the Trellis task directory already captures the version scope, PRD, and verification plan. If GitHub tracking is in use for this task, also confirm the related Issue / PR state is consistent.
...
Research belongs in `{task_dir}/research/*.md`. In this repository's Codex flow, main-session research is allowed by default as long as findings are written to files; use `trellis-research` only when the user explicitly requests parallel/delegated research.
[/workflow-state:planning]

[workflow-state:in_progress]
Flow: trellis-before-dev/main-session implement → trellis-check → trellis-update-spec → finish
Next required action: inspect conversation history + git status, then execute the next uncompleted step in that sequence.
For this repository's Codex flow, the default is to edit in the main session after reading the task + PRD + relevant specs. Use Trellis sub-agents only when the user explicitly asks for parallel/delegated work and the environment supports the required Trellis agent type.
If you do spawn sub-agents, use the exact Trellis agent type names: `trellis-implement`, `trellis-check`, or `trellis-research`. Generic/default/generalPurpose sub-agents do not receive `implement.jsonl` / `check.jsonl` injection.
[/workflow-state:in_progress]
...
Before archive, confirm the repository's Trellis records are complete for the task. If this task also used GitHub Issue / PR tracking, confirm that lifecycle before archive as well.
