...
> 历史规划（2026-09-10）：用户决定结束旧发布治理与图片工作流任务，后续需求改由跨仓统一项目规划与 Issue 确定。以下版本范围和节奏只保留历史参考，不表示已实施，也不自动续作。
...
* 本文保留原跨版本设想，不承担当前任务管理。
* 后续需求、范围与验收以统一项目规划及确认后的 Issue 为准。
* 当前工程规则从 `docs/README.md` 进入，不再引用旧任务与 spec 作为执行合同。
...
* 后续开发使用 Solo，不为每个版本机械创建 task、PRD 或 journal。
* 真实长期需求、blocker 与跨仓依赖使用 Issue。
* 稳定工程规则按需维护在项目文档中，不恢复自动 spec 注入。
...
旧 `v2.4.0 发布治理版` 执行任务已按用户决定结束，由后续统一规划取代。原发布字段、SEO 与图片工作流设想不在本次治理中实现，也不自动转成新的独立需求。
